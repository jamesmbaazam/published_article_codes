...
SEIR <- function(t, x, parms){
  ...
  with(as.list(c(parms,x)),{
    Inv <-   ifelse(t<22,Inv,105000)
    Inv <-   ifelse(t<23,Inv,104009)
    Inv <-   ifelse(t<24,Inv,95459)
    Inv <-   ifelse(t<25,Inv,105000)
    Inv <-   ifelse(t<26,Inv,105000)
    Inv <-   ifelse(t<27,Inv,100859)
    Inv <-   ifelse(t<28,Inv,98610)
    Inv <-   ifelse(t<29,Inv,105000)
    Inv <-   ifelse(t<30,Inv,97528)
    Inv <-   ifelse(t<31,Inv,95459)
    Inv <-   ifelse(t<32,Inv,99690)
    Inv <-   ifelse(t<33,Inv,103919)
    Inv <-   ifelse(t<34,Inv,105000)
    Inv <-   ifelse(t<35,Inv,100769)
    Inv <-   ifelse(t<36,Inv,0)
    dS2 <- - b*S2*I3
...
dt    <- seq(0,180,0.1)      # set the time points for evaluation
...
write.xlsx(out, "C:/Users/oresths/Desktop/out.xlsx")